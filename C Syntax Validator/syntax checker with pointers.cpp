...
void chk(s2 file[],int*intf,int *charf,int *curve, int*prntf ,int*scnf,int*fpts,int*fgts ,int*ffloat, int*fscnf,
         int*curly, int*straight,int*flot, int*nt, int*chr, int*pts,int*gts, int* vod);
...
main()
{
    ...
    int lc=0;
    char d[901];
    file_reading(file, &lc);

    int i=0,k=0,j=1,m,intf=0,charf=0,prntf=0,scnf=0,fpts=0,fgts=0,ffloat=0,fscnf=0,curly=0,curve=0,fabs=0;

    int straight=0,flot=0,nt=0,chr=0,pts=0,gts=0,vod=0;



    chk(file,&intf,&charf,&curve, &prntf ,&scnf,&fpts,&fgts ,&ffloat, &fscnf, &curly, &straight,&flot, &nt, &chr, &pts,&gts, & vod, &fabs );

    printf("\n\n\nBASIC INFORMATION ABOUT THE ABOVE CODE\n=========================================");
    printf("\n\tTotal num of lines-->\t\t        %d",lc);
    printf("\n\tTotal built in functions used-->\t%d",prntf+scnf+fscnf+fgts+fpts);
    ///printf("\n\tTotal num of variables used-->\t\t%d",chr+flot+nt-intf-ffloat-charf);
    ...
    if(charf>0)printf("\n\tTotal number of \"Char\" fuctions-->     %d",charf);
    ...
    //if(fabs>0)printf("\n\tTotal number of \"abs\" function-->          %d",fabs);
    ...
    if(flot>0)
       printf("\n\tThe number of floats variables used are ---> %d",flot-ffloat);
    else
...
void chk(s2 file[],int*intf,int *charf,int *curve, int*prntf ,int*scnf,int*fpts,int*fgts ,int*ffloat, int*fscnf, int*curly, int*straight,int*flot, int*nt, int*chr, int*pts,int*gts, int* vod)
{
	int l,i,k,m;
	for(i=0;i<500;i++)
    {
		for(k=0;k<file[i].ln;k++)
		{
/*scanf*/	if(file[i].l[k]=='s'&&file[i].l[k+1]=='c'&&file[i].l[k+2]=='a'&&file[i].l[k+3]=='n'&&file[i].l[k+4]=='f'&&file[i].l[k+5]=='(')
            {
                 *scnf+=1;
            }
/*printf*/	if(file[i].l[k]=='p'&&file[i].l[k+1]=='r'&&file[i].l[k+2]=='i'&&file[i].l[k+3]=='n'&&file[i].l[k+4]=='t'&&file[i].l[k+5]=='f'&&file[i].l[k+6]=='(')
            {
                  *prntf+=1;
            }
/*fscanf*/	if(file[i].l[k]=='f'&&file[i].l[k+1]=='s'&&file[i].l[k+2]=='c'&&file[i].l[k+3]=='a'&&file[i].l[k+4]=='n'&&file[i].l[k+5]=='f'&&file[i].l[k+6]=='(')
            {
                  *fscnf+=1;
            }
/*fgets*/	if(file[i].l[k]=='f'&&file[i].l[k+1]=='g'&&file[i].l[k+2]=='e'&&file[i].l[k+3]=='t'&&file[i].l[k+4]=='s'&&file[i].l[k+5]=='(')
             {
                 *fgts+=1;
             }
/*fputs*/	if(file[i].l[k]=='f'&&file[i].l[k+1]=='p'&&file[i].l[k+2]=='u'&&file[i].l[k+3]=='t'&&file[i].l[k+4]=='s'&&file[i].l[k+5]=='(')
            {
                 *fpts+=1;
            }
/*gets*/	if(file[i].l[k]=='g'&&file[i].l[k+1]=='e'&&file[i].l[k+2]=='t'&&file[i].l[k+3]=='s'&&file[i].l[k+4]=='(')
            {
                *gts+=1;
            }
/*puts*/	if(file[i].l[k]=='p'&&file[i].l[k+1]=='u'&&file[i].l[k+2]=='t'&&file[i].l[k+3]=='s'&&file[i].l[k+4]=='(')
            {
                *pts+=1;
            }
